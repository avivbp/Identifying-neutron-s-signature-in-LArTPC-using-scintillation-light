   ...
   fDetectorMessenger(nullptr),lvol_fiber(nullptr),pvol_fiber(nullptr)
{
  ...
  G4double length = 3000.*cm;
  absorptionLength = {length, length, length};
  G4double raylength = 3.*cm;
  std::vector<G4double> rayleighLength = {raylength,raylength,raylength};
  ...
  MPT->AddProperty("RAYLEIGH",energy, rayleighLength);
...
{ 
  ...
  // ARGON CELL
  G4double lArDiameter = 21.*cm;
  G4double lArLength = 23.5*cm;
 
  auto* Acrylic = G4NistManager::Instance()->FindOrBuildMaterial("G4_POLYACRYLONITRILE");

  fSolidAcrylic = new G4Tubs("Acrylic",0.5*lArDiameter,0.5*lArDiameter+1.9*cm,0.5*lArLength,0.,twopi);
    
  fLogicAcrylic = new G4LogicalVolume(fSolidAcrylic,    //its solid
  ...
  fPhysiAcrylic = new G4PVPlacement(transform1,
                     fLogicAcrylic,     //its logical volume
                       "Acrylic",         //its name
                         airLogic,        //its mother
  ...
  std::cout << "inner vessel half length  = " << fSolidAbsorber->GetZHalfLength() << std::endl;                         
  ...
  G4LogicalBorderSurface("name",fPhysiAbsorber,fPhysiAcrylic,OpSurface);
 ...
 visAttributes = new G4VisAttributes(G4Colour(1.0,1.0,0.0));   // make Acrylic Layer yellow
 fLogicAcrylic->SetVisAttributes(visAttributes);
 fVisAttributes.push_back(visAttributes);
