  ...
  fParticle(0), fEkin(0.)
{
  ...
  csvfi.open("VitaliyStats.csv");
  csvfi << "eventID" << "," << "recoilEnergy" << "," << std::endl;
  csvfi.close();

  csvfi.open("numPhotSec.csv"); 
  csvfi << "eventID" << "," << "numPhot" << "," << std::endl;
  csvfi.close();
 ...
 // csvf << "reaction" << "," << std::endl;
  ...
  fNbGamma = fNbElect = fNbPosit = numSuc = numBackground = 0;
  single_elastic = multiple_elastic = elastic_NNPrime = multiple_NNPrime = no_interaction = 0;
