...
// std::cout << passed << std::endl;
 ...
 nScint = nCher = numPhotons = largestTrackID = lastTracked = numReachedUp = numReachedDown = numPE = num = 0;
 fTransmitFlag   = fReflectFlag    = scatteredNotSensitive = extInelastic = Coincedence = fail = numInelastic = 0;    
...
void EventAction::EndOfEventAction(const G4Event* evt)
{
 ...
 // std::cout << "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" << std::endl;
  ...
  //run->csvfi << eventID << "," << num << "," << std::endl;
  //run->csvfi.close();
 ...
 // std::cout << "eDep = " << enDep << " keV" << std::endl;
  ...
  if (detected && (numElasticSensitive >= 1 || scatteredInelastically) && ((tOne - tZero) > 0.*CLHEP::ns) && ((tOne-tZero) < 100.*CLHEP::ns)){
      //run->csvfi.open("experimentalEnergies.csv",std::ios_base::app);
      if (!fail && detected && numElasticSensitive == 1){
          //run->csvfi << eventID << "," << nucleusRecoilEnergy << "," << tOne - tZero << "," << 1 << "," << std::endl;
      }
      else if (fail && detected && !scatteredInelastically){
          //run->csvfi << eventID << "," << nucleusRecoilEnergy << "," << tOne - tZero << "," << 0 << "," << std::endl;
      }
      //run->csvfi.close();
  }
