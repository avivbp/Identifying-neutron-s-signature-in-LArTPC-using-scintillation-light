 ...
 // }
  ...
  if(particleDef == opticalphoton){
      ...
      if(track->GetCreatorProcess()->GetProcessName() == "Scintillation" ){
          ...
          if (processName == "Transportation" && aStep->GetPreStepPoint()->GetTouchableHandle()->GetVolume()->GetName() == "biggerAbsorber" && trackID != fEventAction->lastTracked && pos.x() == 50){
  ...
  if (incomingParticleName == "neutron" && volumeName == "biggerAbsorber" && processName == "hadElastic"){
      fEventAction->numElastic += 1;
  ...
  if (incomingParticleName == "neutron" && volumeName == "biggerAbsorber" && processName == "neutronInelastic"){
  ...
  //if (trackID == 1 && volumeName == "biggerAbsorber" && trans.compare(processName) == 0 && aStep->GetTrack()->GetMomentumDirection() != G4ThreeVector(1,0,0)){ 
      //std::cout << "went through detector" << std::endl;
     // printEventStats(aStep);
  //}
