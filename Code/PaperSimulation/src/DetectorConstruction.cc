   ...
   fDetectorMessenger(nullptr),lvol_fiber(nullptr),pvol_fiber(nullptr)
{
  ...
  MPT->AddConstProperty("SCINTILLATIONYIELD", 51000./MeV);
  ...
  // Doke et. al - Birks constant which fits 1MeV electron scintillation data
  //lAr->GetIonisation()->SetBirksConstant(0.89* mm / MeV);

  // playing around with Birk's constant to match with Creus et al. Data
  lAr->GetIonisation()->SetBirksConstant(0.1* mm / MeV);
...
{ 
 ...
 // fSolidAbsorber = new G4Box("Absorber",        
  ...
  biggerSolidAbsorber = new G4Tubs("lAr",0.*cm,0.5*biggerlArDiameter,0.5*biggerlArLength,0.,twopi);
  ...
  position2 = G4ThreeVector(1.*m,0,0);
  ...
  // some material to create reflective surface only on the casing of the liquid scintillator, not on the bases
  sreflect_fiber = new G4Tubs("reflectfiber",                      //name
                        0.5*fiberDiameter, 0.5*fiberDiameter+1.5*mm,       //r1, r2
                        0.5*fiberLength,               //half-length
                        0., twopi);                    //theta1, theta2

  lreflect_fiber = new G4LogicalVolume(sreflect_fiber,          //solid
                                  Al,            //material
                    "reflectfiber");            //name

  preflect_fiber = pvol_fiber = new G4PVPlacement(transform2,
                                lreflect_fiber,                     //its logical volume
                                "reflectfiber",           //its name
                                airLogic,                   //its mother
                                false,                         //no boulean operat
                                0);
 
  G4OpticalSurface* OpSurface2 = new G4OpticalSurface("name2");

  G4LogicalBorderSurface* Surface2 = new
  G4LogicalBorderSurface("name2",pvol_fiber,preflect_fiber,OpSurface2);

  OpSurface2->SetType(dielectric_dielectric);
  OpSurface2->SetModel(unified);
  OpSurface2->SetFinish(groundfrontpainted);
  //OpSurface->SetSigmaAlpha(0.1);
  ...
  //SMPT->AddProperty("SPECULARSPIKECONSTANT", pp, specularspike);
  //SMPT->AddProperty("BACKSCATTERCONSTANT", pp, backscatter);
  SMPT2->AddProperty("REFLECTIVITY", pp, reflectivity);
  SMPT2->AddProperty("EFFICIENCY", pp, efficiency);
  ...
  std::cout << "position before rotation = " << position2 << std::endl;
  position2 = rotateAroundY(position2,0.4363);
  std::cout << "position after rotation = " << position2 << std::endl;
 ...
 auto visAttributes = new G4VisAttributes(G4Colour(0.,0.,0.)); // world is white
 ...
 visAttributes = new G4VisAttributes(G4Colour(1.0,1.0,0.0));   // make Aluminium Layer of fiber reflect yellow
 lreflect_fiber->SetVisAttributes(visAttributes);
 fVisAttributes.push_back(visAttributes);
