...
{    
  ...
  G4double yDir = (2*G4UniformRand()-1.)*0.029;
  G4double zDir = (2*G4UniformRand()-1.)*0.029;
  fParticleGun->SetParticleMomentumDirection(G4ThreeVector(1.,0.,0.));
  ...
  if (fRndmBeam > 0.) 
    {
     ...
     // std::cout << "primary starting position:" << G4ThreeVector(x0,y0,z0)/CLHEP::cm << std::endl;
      ...
      G4double yDir = (2*G4UniformRand()-1.)*0.029;
      G4double zDir = (2*G4UniformRand()-1.)*0.029;
      fParticleGun->SetParticleMomentumDirection(G4ThreeVector(1.,yDir,zDir));
    }
