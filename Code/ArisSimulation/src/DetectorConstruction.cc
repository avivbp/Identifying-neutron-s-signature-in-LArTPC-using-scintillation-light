   ...
   fDetectorMessenger(nullptr),lvol_fiber(nullptr),pvol_fiber(nullptr)
{
  ...
  absorptionLength = {60.*cm, 60.*cm, 60.*cm};
...
{ 
  ...
  auto* hydrogen = G4NistManager::Instance()->FindOrBuildMaterial("G4_H");
  //hydroSolid = new G4Box("air",
    //                   5.*cm,5.*cm,5.*cm);
  //hydroLogic = new G4LogicalVolume(hydroSolid,    //its solid
    //                             hydrogen,
      //                           "hydrogen");

  //hydroPhysi = new G4PVPlacement(0,                      //no rotation
    //                          G4ThreeVector(-1.*m,0.,0.),
      //                        hydroLogic,             //its logical volume
        //                      "hydrogen",                 //its name
          //                    airLogic,                       //its mother  volume
            //                  false,                   //no boolean operation
              //                0);

  // ARGON CELL
  G4double lArDiameter = 76.*mm;
  G4double lArLength = 78.7*mm;
 
  auto* Teflon = G4NistManager::Instance()->FindOrBuildMaterial("G4_TEFLON");

  fSolidTeflon = new G4Tubs("Teflon",0.5*lArDiameter,0.5*lArDiameter+1.*cm,0.5*lArLength,0.,twopi);
  ...
  fPhysiAbsorber = new G4PVPlacement(transform1,
  ...
  G4LogicalBorderSurface("name",fPhysiAbsorber,fPhysiTeflon,OpSurface);
 ...
 G4double angleArr[8] = {0.4451,0.6248,0.7191,0.7976,1.1205,1.4923,1.9757,2.3230};
 G4double distances[8] = {2.5*m,2.5*m,1.5*m,1.3*m,1.3*m,1.3*m,1.3*m,1.3*m};
 G4ThreeVector vectArr[8];
 for(int i = 0;i<=7;i++){
     if (i == 2){
         vectArr[i] = G4ThreeVector(distances[i],-0.2*m,0.);
     }
     else{
 ...
 //std::cout << "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA" << std::endl;
 for (int i = 0;i<=7;i++){
     std::stringstream ss;
     ss << "A" << i;
     std::string fibName = ss.str();
     std::cout << fibName << std::endl;
     G4LogicalVolume* volume = G4LogicalVolumeStore::GetInstance()->GetVolume(fibName);

     if (volume) {
        // Create visualization attributes
        G4VisAttributes* visAttributes = new G4VisAttributes(G4Colour(0.19,0.83,0.78)); // turquoise color
        visAttributes->SetForceSolid(true);
        fVisAttributes.push_back(visAttributes);
     } else {
        G4cerr << "Error: Volume 'MyVolume' not found!" << G4endl;
       }
 }
